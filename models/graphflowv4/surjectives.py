...
from .argmax import ArgmaxSurjection, ConditionalAdjacencyBlockFlow, ar_net_init
...
class ContextNet(nn.Module):
    def __init__(self, context_size=16, num_classes=4, embedding_dim=7, max_nodes=9, hidden_dim=32):
        super(ContextNet, self).__init__()
        #Assume input B x 45
        self.net = nn.Sequential(
            nn.Embedding(num_classes, embedding_dim), # B x 45 x embedding_dim
            Rearrange("B N E -> B (N E)"),
            nn.LazyLinear(max_nodes * embedding_dim),
            nn.ReLU(),
            Rearrange("B (N E) -> B 1 N E", N=max_nodes, E=embedding_dim),
...
class AtomSurjection(Surjection):
    ...
    def __init__(self, num_classes=6, max_nodes=9, hidden_dim=64, mask_ratio=9., block_length=6):
        ...
        context_net = ContextNet(context_size=context_size, num_classes=num_classes, max_nodes=max_nodes, embedding_dim=embedding_dim, hidden_dim=hidden_dim)
